...
const toSupabase = (item: Item, userId: string) => ({
    ...
    repeat_config: item.repeatConfig ? (typeof item.repeatConfig === 'string' ? JSON.parse(item.repeatConfig) : item.repeatConfig) : null,
    skipped_dates: item.skippedDates ? (typeof item.skippedDates === 'string' ? JSON.parse(item.skippedDates) : item.skippedDates) : null,
    completed_dates: item.completedDates ? (typeof item.completedDates === 'string' ? JSON.parse(item.completedDates) : item.completedDates) : null,
